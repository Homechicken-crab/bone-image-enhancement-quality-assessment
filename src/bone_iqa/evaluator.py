...
def _metric_status(row: dict[str, Any]) -> tuple[str, str]:
    missing = []
    if row.get("weak_bone_mean_cnr_background") is None:
        missing.append("Weak Bone Background CNR")
    if row.get("weak_bone_mean_cnr_local") is None:
        missing.append("Weak Bone Local CNR")
    if row.get("weak_bone_mean_average_gradient") is None:
        missing.append("Weak Bone Average Gradient")
    if row.get("background_noise_pooled") is None:
        missing.append("Background Noise")
    if row.get("strong_bone_saturation_mean") is None:
        missing.append("Saturation Ratio")
    return ("valid", "") if not missing else ("partial", "不可计算：" + "、".join(missing))
...
def _evaluate_image(
    ...
    for roi in rois:
        ...
        if row["cnr_background"] is None or row["cnr_local"] is None:
            ...
            if row["cnr_background"] is None:
                reasons.append("Background CNR 无效")
            if row["cnr_local"] is None:
                reasons.append("Local CNR 无效")
    ...
    summary["status"], summary["message"] = _metric_status(summary)
    ...
    for record in records:
        ...
        if record.role == "original":
            ...
            if score is None:
                ...
                summary["message"] = (summary.get("message", "") + "；SSIM 不可计算").strip("；")
